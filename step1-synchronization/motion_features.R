...
motion_features <- function(ds, who) {
  ...
  if (who == "infant") {
    ...
    sensor_diffs <- map_dfc(sensor_features, 
                            ~ t(list(diff12 = diff12(.x), diff13 = diff13(.x), diff14 = diff14(.x), diff23 = diff23(.x), diff24 = diff24(.x), diff34 = diff34(.x)))) %>% 
      set_names(simplify(map(sensor_names, ~ paste0(diff_fx[seq_along(diff_fx)],"_", .x))))
    ...
    sensor_absdiffs <-  map_dfc(sensor_features, 
                                ~ t(list(diff12 = diff12(.x %>% mutate(across(everything(), abs))),
                                         ...
                                         diff34 = diff34(.x %>% mutate(across(everything(), abs)))))) %>% set_names(simplify(map(sensor_names, ~ paste0("ABS",diff_fx[seq_along(diff_fx)],"_", .x))))
  }
