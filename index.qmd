...
younger <- filter(dashboard, agemo <= 8)
older <- filter(dashboard, agemo > 8)
```
...
| There are `r length(unique(younger$id))` unique younger participants with `r nrow(younger)` overall sessions. 
| There are `r length(unique(older$id))` unique older participants with `r nrow(older)` overall sessions. 
...
dash_filt <- dashboard %>% select(-sessions_dir) %>% 
  ...
  rename(Paperwork = `Completed Paperwork`, Biostamp = `Biostamp Annotations`, `LENA ITS` = `LENA Downloaded`, Age = Agemo)
...
reactable(dash_filt, 
          ...
          columnGroups = list(colGroup("Visit", columns = c("ID", "Session", "Age"), sticky = "left")),
          columns = list(
            ...
            Age = colDef(format = colFormat(digits = 1), minWidth = 70),
            Date = colDef(format = colFormat(date = TRUE), minWidth = 130),
            `Start Time` = colDef(minWidth = 130),
            `End Time` = colDef( minWidth = 130),
            ...
            `IMU Files` = colDef(style = function(value) {if (value > 3) list(color = "#008000") else list(color = "#e00000", fontWeight = "bold")}, format = colFormat(digits = 0)),
            ...
            `Position Primary` = colDef(style = function(value) {if (value > 15) list(color = "#008000") else list(color = "#e00000", fontWeight = "bold")}, format = colFormat(digits = 0)),
            `Position Rel` = colDef(style = function(value) {if (value > 15) list(color = "#008000") else list(color = "#e00000", fontWeight = "bold")}, format = colFormat(digits = 0)),
            `Activity Exported` = colDef(cell = function(value) {if (value > 0) "\u2714\ufe0f" else "\u274c"}),
            `Biostamp` = colDef(cell = function(value) {if (value > 0) "\u2714\ufe0f" else "\u274c"}),
            `Infant Synced Old` = colDef(cell = function(value) {if (value > 0) "\u2714\ufe0f" else "\u274c"}),
            `Infant Synced Jul` = colDef(cell = function(value) {if (value > 0) "\u2714\ufe0f" else "\u274c"}),
            `Infant Synced Codes` = colDef(cell = function(value) {if (value > 0) "\u2714\ufe0f" else "\u274c"}),
            ...
            `Pt2 Processed` = colDef(cell = function(value) {if (value > 0) "\u2714\ufe0f" else "\u274c"})
          )
          )
```
